...
vim.api.nvim_create_user_command("PopupEcho", function(opts)
  local out = vim.fn.systemlist("echo " .. (opts.args ~= "" and opts.args or "hello"))
  local buf = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_buf_set_lines(buf, 0, -1, false, out)

  local cols, lines = vim.o.columns, vim.o.lines
  local width  = math.max(40, math.floor(cols * 0.45))   -- ~45% of screen
  local height = math.max(10, lines - 4)                  -- tall window
  local row, col = 1, cols - width - 1                    -- right side
...
vim.api.nvim_create_user_command("PopupTerm", function(opts)
  local cols, lines = vim.o.columns, vim.o.lines
  ...
  local cmd = (opts.args ~= "" and opts.args) or (vim.o.shell .. " -l")
  vim.fn.termopen(cmd, { on_exit = function() pcall(vim.api.nvim_win_close, win, true) end })
  vim.cmd("startinsert") -- enter terminal-mode so keys go to the program

  -- Esc leaves terminal-mode; 'q' (in normal mode) closes the popup
